...
- The notarized `0.1.0 (2)` candidate passed artifact validation but is superseded before publication: keyboard QA found that Quick Actions did not reliably focus its search field. The fix is assigned build `3` and must ship only as a newly notarized `v0.1.0-beta.2` candidate.
...
- GitHub Actions validated the superseded beta.1 DMG on clean ARM64 and Intel runners. Primary-Mac QA passes for installation, first launch, Settings, menu bar, Storage pre-consent, AI Workloads, Light/Dark, 980×680, and 1180×800; the exact beta.2 artifact must repeat the applicable gates before publication.
